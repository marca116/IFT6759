...
dataset_name = 'qald_unique_entities' # qald_9_plus_train_wikidata and qald_10_test
entities_ids_filename = dataset_name + ".txt"
entities_ids_with_urls_filename = dataset_name + "_with_urls.csv"

unique_entities_ids = []

with open(entities_ids_filename, 'r', encoding='utf-8') as file:
    lines = file.readlines() 

    for x, line in enumerate(lines, start=1):
        entity_id = line.strip()
        unique_entities_ids.append(entity_id)
...
entity_ids_groups = []
for x in range(0, len(unique_entities_ids), max_entity_per_group):
    entity_ids_group = unique_entities_ids[x:x+max_entity_per_group]
...
for result in missing_entities_and_url:
    entity_id, entity_label, title, wiki_url = result
    with open(entities_ids_with_urls_filename, 'a', encoding='utf-8', newline='') as f:
